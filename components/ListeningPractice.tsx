  ...
  return (
    <div className="max-w-2xl mx-auto py-10 px-4 animate-in fade-in">
      <div className="flex items-center justify-between mb-10">
        <button onClick={onExit} className="text-slate-400 font-bold flex items-center">← Thoát</button>
        <div className="flex gap-2">
          <button onClick={() => setPlaybackSpeed(1.0)} className={`px-4 py-1.5 rounded-full text-xs font-black ${playbackSpeed === 1.0 ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-400'}`}>1.0x</button>
          <button onClick={() => setPlaybackSpeed(0.7)} className={`px-4 py-1.5 rounded-full text-xs font-black ${playbackSpeed === 0.7 ? 'bg-orange-500 text-white' : 'bg-slate-100 text-slate-400'}`}>0.7x 🐢</button>
        </div>
      </div>
      ...
      <div className={`bg-white rounded-[3rem] p-12 shadow-2xl border-2 transition-all ${showResult === 'correct' ? 'border-green-400' : showResult === 'incorrect' ? 'border-red-400' : 'border-slate-50'}`}>
        <div className="text-center mb-10">
          <button onClick={handleSpeak} disabled={isSpeaking} className={`w-32 h-32 rounded-full flex items-center justify-center mx-auto mb-6 shadow-xl ${isSpeaking ? 'bg-indigo-100' : 'bg-indigo-600 text-white'}`}>
            {isSpeaking ? "🔊..." : "▶️"}
          </button>
          <p className="text-indigo-600 font-black text-xl">Nghe và gõ lại</p>
        </div>
        ...
        <button onClick={showResult === 'idle' ? handleCheck : nextQuestion} className="w-full py-5 rounded-2xl bg-slate-900 text-white font-black">{showResult === 'idle' ? 'Kiểm tra' : 'Tiếp theo'}</button>
      </div>
    </div>
  );
};
